...
export default function BkPage() {
  ...
  // 「一覧を表示」クリック：サイズ限定テーブル（25ml中性緩衝）を表示＋データ取得
  ...
  // サイズ限定リスト
  const bkOnlyList = formalinList.filter((f) => f.size === SIZE);
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold mt-4 ml-10">25ml中性緩衝 専用ページ</h1>
      ...
      {/* サイズ=25ml中性緩衝 の FormalinTable（履歴ボタン付き） */}
      {showSizeTable && (
        <div className="bg-white shadow rounded p-4">
