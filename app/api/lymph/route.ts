...
async function countOrOutAtTime(atUtc: Date, lowerBoundUtc: Date) {
  const records = await prisma.history.findMany({
    where: {
      updated_at: { gte: lowerBoundUtc, lte: atUtc }, // ★ 2ヶ月ウィンドウに限定
      formalin: { size: SIZE },
    },
  ...
  for (const r of records) {
    ...
    if (seen.has(r.formalinId)) continue; // formalinIdごとに最新1件だけ
...
export async function POST(request: Request) {
  try {
    ...
    if (error instanceof Error) {
      console.error("POST /api/lymph error:", error.message);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }
    console.error("POST /api/lymph unknown error:", error);
    return NextResponse.json({ error: "Failed to fetch lymph data" }, { status: 500 });
  }
}
