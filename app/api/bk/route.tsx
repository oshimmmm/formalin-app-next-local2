...
export async function POST(request: Request) {
  try {
    ...
    if (error instanceof Error) {
      console.error("POST /api/bk error:", error.message);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }
    console.error("POST /api/bk unknown error:", error);
    return NextResponse.json({ error: "Failed to fetch bk data" }, { status: 500 });
  }
}
